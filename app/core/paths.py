"""Canonical runtime-path policy for NiuOne.

Runtime data lives in an ignored ``.local-data`` directory by default so a
repository upload does not accidentally commit databases, tokens, logs, or
generated reports.
"""
...
def container_runtime_overrides(root: Path) -> dict[str, str]:
    ...
    app_dir = root / "app"
    entrypoint_dir = app_dir / "entrypoints"
