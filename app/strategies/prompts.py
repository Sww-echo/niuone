...
9. 外部确认只做限幅覆盖：读取已完成的隔夜美股盘面及明确A股行业映射，并对首轮前5候选读取近3日个股消息面。正向外盘/消息不能把落后行业、追高或硬过滤候选变成买点；隔夜防守、负行业映射和明确利空必须降权并写入风险。""" if sector_tide_enabled else ""
    ...
    niuone_strategy_section = f"""牛牛战法（主线酝酿→主线主升→主线高潮→主线分歧→主线退幕）：
所有阶段的牛牛买入（含试仓、启动、领涨、转强及加仓）都须满足个股当日实际累计换手率≥{NIUONE_MIN_ENTRY_TURNOVER_PCT:g}%、全市场成交额分位≥{NIUONE_MATURE_MIN_MARKET_AMOUNT_PERCENTILE:g}、题材内成交额分位≥{NIUONE_MATURE_MIN_THEME_AMOUNT_PERCENTILE:g}；任何必需数据无效就暂停买入，不用预计全天换手替代。早盘未达标等待，退幕仍只退出。
...
7. 进攻/轮动/修复/防守的确认路径单笔权益风险≤1.50%/1.00%/0.60%/0.30%、主题风险≤3.00%/2.00%/1.20%/0.60%、主题敞口≤55%/40%/25%/12%；牛牛试仓仅≤0.35%/1.00%/0.25%/0.15%、主题风险≤0.70%/1.00%/0.50%/0.30%、主题敞口≤12%/10%/8%/5%。策略内组合未实现止损风险≤4.50%/3.00%/1.80%/0.90%，总仓≤70%/55%/35%/20%；不设固定同板块或同题材持仓只数上限，策略同时最多持有{max_open_positions}只（来自设置中的最大持仓只数）；防守允许开仓，复合风险硬停止才禁止新仓。
