...
2. 阶段与动作分离：candidate及未跨日延续的emerging为酝酿，跨日延续的emerging及普通强度mainline为主升，确认主线且题材分数达到78为高潮，diverging为分歧，fading为退幕；高潮仍允许满足领涨或企稳转强条件的核心股开仓，分歧只观察核心股调整后转强或减仓，持续回落不触发买点，退幕只退出。日内强势不直接升级主线。
...
4. 牛牛转强（兼容旧称承接/回踩）：主题仍为mainline/diverging、尚未退幕且分数≥70时，高潮和分歧均可观察行业前三核心股在EMA20附近企稳后重新收复；持续回落不触发买点。不设固定单日涨幅上限，实际执行价达到涨停才拒绝；进攻行情距EMA20上限为1.25ATR，轮动/修复为1ATR，25%是单票绝对上限。
5. 牛牛试仓（基于日线V型反转，兼容旧称反转）：在未触发复合风险硬停止的candidate/emerging题材中识别最近30根日K内的V型趋势；防守行情仍可按最低风险档开仓。candidate题材里已成为strong强势股的名称必须等待牛牛启动确认，mainline/diverging成熟题材不得继续以试仓身份开仓。左侧回落至少5个交易日且跌幅≥8%，阶段低点距今3～15个交易日，右侧反弹≥6%、收复左侧跌幅须在60%（含）至200%（不含）之间，上涨交易日占比至少2/3并确认右侧继续抬高；达到200%后不再按早期试仓。题材还须至少有6只强势股，或酝酿状态已连续3个交易日。以右侧最近3根日K低点为止损且止损距离不得超过6%/2ATR，距EMA20不得超过1.5ATR；不设固定单日涨幅上限，实际执行价达到涨停才拒绝。每天最多保留评分最高的2只，按早期试仓风险预算参与，单票绝对上限6.25%。
6. 牛牛启动：主题必须是跨交易日延续、但尚未正式确认为主线的emerging；已确认mainline必须改走牛牛领涨，不得再用低门槛启动身份参与。至少两只强势股保持共振，只允许行业前三龙头梯队突破/收复时建立观察仓。突破路径按超过前20日高点的ATR距离复核并以突破位下方0.5ATR为止损，收复路径仍按距EMA20复核；不设固定单日涨幅上限，实际执行价达到涨停才拒绝，对应锚点距离仍不得超过1.5ATR，15%是单票绝对上限。启动仍是观察仓。试仓或启动持仓只有浮盈在2%～12%、生命周期仍为主升且个股保持强势领涨梯队时才分级加仓：启动主线跨日延续先向10%上限加一次，主线完全确认后再向20%上限加一次；实际仓位仍取风险预算和阶段上限的较小值，分歧/高潮/退幕不加仓。
