...
本文记录仓库主页九个功能 GIF 的录制、生成、压缩和验收方式，供功能界面或演示数据更新后复用。

## 目标与约束

- 使用真实页面交互，不用静态截图轮播代替点击、滚动或图表悬停。
- 九个 GIF 统一使用浅色主题，避免同一组素材出现明暗主题混用。
...
- 使用公开演示页、已经加载的只读缓存页面或隔离的临时运行目录，不录制密钥、Webhook、数据库、日志或其他私有运行数据。
...
|---|---|---:|---:|---:|---|
| `overview.gif` | `/` | 1200 × 750 | 8 fps | 7–9 秒 | 展开量能曲线并查看主线代表股 |
| `practice-trading.gif` | `/practice` | 1200 × 750 | 8 fps | 6–8 秒 | 切换每日/累计收益并打开交易日历 |
...
1. 从完整总览开始，同时展示决策摘要、市场情绪、指数、板块资金、主线机会、候选池和财经快讯。
2. 鼠标移动到“预测 / 今昨实际 / 差额”，真实点击并等待量能曲线加入图表。
3. 停留展示量能、红绿盘和涨跌停曲线的联动变化。
4. 鼠标移动到今日排名第一名的核心股入口，真实点击并展开代表股。
5. 再次点击收起，让结尾自然回到总览状态。

画面必须保留顶部决策摘要与下方主要信息模块，不能裁切为单张图表。总览财经快讯只展示投影条目，完整来源切换和滚动浏览由 `realtime-news.gif` 单独展示。
...
2. 保持完整图表卡片为画面主体，同时保留顶部导航上下文；仅在页面高度变化导致图表超出视口时小幅滚动。
...
scrollY: 当前布局为 0
hoverY: 约 480
hoverX: 250 → 1050，步长约 32；再折返至 570
```
...
1. 从“全部”来源的最新聚合快讯开始，保留来源计数、服务状态和列表表头。
2. 鼠标移动到“仅重要”，真实点击并停留，让重要标签和筛选后的列表清晰可见。
3. 点击一个有内容的来源标签，展示单一来源的数量和条目。
4. 连续向下滚动浏览数屏，再连续返回列表顶部。
5. 停留约 1 秒，让筛选状态和来源标签保持可辨认。
...
1. 从浅色主题的“业务配置”总览开始，完整展示主要设置分组及各组配置项数量。
2. 点击“选股与交易策略”，展示独立策略列表和当前状态。
3. 点击另一套策略，让“有未保存修改”状态出现；随后点击原策略恢复初始值，不保存演示改动。
4. 点击“全部设置”返回总览，展示盘面监控、任务调度及行情与资金流设置。
...
推荐控制九个 GIF 总体积在 10 MB 左右。单个文件明显偏大时，依次尝试：
...
cp "$GIF_WORK_DIR/overview.gif" docs/assets/readme/overview.gif
