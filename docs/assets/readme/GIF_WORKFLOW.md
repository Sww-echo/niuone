...
本文记录仓库主页八个功能 GIF 的录制、生成、压缩和验收方式，供功能界面或演示数据更新后复用。

## 目标与约束

- 使用真实页面交互，不用静态截图轮播代替点击、滚动或图表悬停。
- 八个 GIF 统一使用暗色主题，避免同一组素材出现明暗主题混用。
...
| `theme-strength.gif` | `/niuone-mainline` | 1200 × 675 | 8 fps | 8–10 秒 | 对照今日/结构排名，分别展开代表股并查看有效覆盖原因 |
...
录制视口为 1600 × 900，输出为 1200 × 675，保持标准 16:9。

1. 从题材强度页开始，让跨日确认主线、今日领涨题材、市场状态和有效覆盖四张摘要卡完整可见，同时保留并排的“今日排名”和“结构排名”。
2. 点击今日排名第一名的“领涨”入口，展开今日领涨股、代码、归因权重和当前涨跌幅，停留约 1 秒。
3. 点击结构排名第一名的“龙头”入口，关闭前一个浮层并展开结构代表股，停留约 1 秒。
4. 点击“题材有效覆盖”旁的信息按钮，展示互斥归类的未覆盖原因和数量，停留约 1 秒。
5. 点击标题区域关闭浮层，回到双榜总览，让循环自然衔接。

摘要卡、双榜和展开内容必须同时处于可读范围；今日榜中的东财即时排名、上涨广度和领涨股应清晰可见。东财字段只用于交叉验证，不能在说明中表述为改变题材排名或交易门槛。不要裁切为只剩单张卡片，也不要用静态截图轮播代替真实点击。
...
3. 停留约 1 秒，同时展示板块涨跌幅与成交额、换手率、成交量活跃股票榜。
4. 连续向下滚动，展示主力净流入前十和主力净流出前十，再回到页面顶部。
5. 点击“资金流动”，等待左侧净流出行业与右侧净流入行业完整出现。
6. 选择播放速度并点击“重播”，连续截取进度线和柱状图变化。
...
推荐控制八个 GIF 总体积在 8 MB 左右。单个文件明显偏大时，依次尝试：
...
cp "$GIF_WORK_DIR/theme-strength.gif" docs/assets/readme/theme-strength.gif
