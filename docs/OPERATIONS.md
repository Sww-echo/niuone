...
| `DASHBOARD_NIUONE_FORWARD_COHORT_START` | `2026-09-03` | 严格前向队列纳入首次 BUY 的起始交易日 | 后续 Cron 周期读取；改值后必须建立新协议锁 |
