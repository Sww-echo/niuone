...
自选股按每只股票约 1 万元、一股取整的原规则进行独立观察，不计交易费用。
展示 5/7/15/60 日只改变历史列；当前筛选内的持仓成本与估值使用完整有效行情历史，
不随展示窗口改变。缺少报价的已建仓股票仍计入成本，组合市值和盈亏显示待补齐；
较早的有效报价可以用于估值，但页面会标出日期。分组按钮的数量和收益始终对应完整分组。

重复添加代码会跳过已有记录，保留其分组、备注、买入日期和数量。只有明确修改买入日期
或金额才重算买入；行情回补不会改写已保存的买入。默认买入日为北京时间最近已收盘的
交易日。报价保存来源交易时间，页面分别显示行情截至日期与页面刷新时间；缺失值显示
`—`，真实零值正常显示。日期校验复用本地 A 股交易日历，日历缺失时沿用工作日降级，
看板读取不触发网络日历刷新。旧的周末、未来日期报价不用于展示或估值，也不自动删除。

新的盘中报价会标为临时数据，不用来建立按收盘价计算的买入。收盘后“只补缺失日”也会
补齐临时报价和缺失价格；已完成的有效历史报价保持原样。升级会为自选股数据库追加
报价时间、收盘状态和任务表，保留原记录；旧日线缺少时间信息时沿用已有日线口径。
部署前备份私有的 `watchlist_tracker.db`（配置了 `DASHBOARD_WATCHLIST_DB` 时使用该路径）。

“更新今日行情”和“回补历史行情”提交后立即返回任务编号，单个 Dashboard 进程内一次
只执行一个行情任务；网络调用沿用现有有界超时与重试。页面显示进度、耗时、成功、跳过
和失败项，重新打开页面会恢复最近任务。后台状态在同一私有数据库内保留最近 20 次；
服务重启会将未完成任务标为中断，管理员可以只重试失败及未完成的股票。成功项不会被
重新排入这次重试，已移除或停用的股票会跳过。前端读取进度失败不代表后台工作已取消。

API：`POST /api/watchlist/jobs`（`kind` 为 `update-today` 或 `backfill`）返回 202；
`GET /api/watchlist/jobs/latest`、`GET /api/watchlist/jobs/{id}` 读取状态；
`POST /api/watchlist/jobs/{id}/retry` 重试。任务提交、重试及已有写接口仍须管理员认证，
已有同步行情接口保持兼容；看板和任务摘要的读取不使用公共缓存。回补天数须为 1～365。
