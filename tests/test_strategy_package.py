...
class StrategyPackageTests(unittest.TestCase):
    ...
    def test_niuone_prompt_matches_execution_risk_budget_and_position_caps(self):
        ...
        self.assertIn("不设每日候选数量上限", active)
        self.assertIn("实际执行价相对有效前收盘价的涨幅必须小于3%", active)
        self.assertIn("不得用扫描价格或涨幅替代", active)
        self.assertIn("已有持仓加仓另按原规则", active)
        self.assertIn("个股当日实际累计换手率≥3%", active)
        self.assertIn("全市场成交额分位≥60", active)
        self.assertIn("题材内成交额分位≥50", active)
        self.assertIn("不用预计全天换手替代", active)
