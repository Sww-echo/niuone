...
class TradeNotificationSettingsTests(unittest.TestCase):
    ...
    def test_notification_test_uses_unsaved_values_and_saved_secret_fallbacks(self):
        ...
        message = captured["notification"].plain_text()
        self.assertIn("通知渠道测试", message)
        self.assertIn("飞书 渠道配置验证消息", message)
        self.assertIn("发送时间", message)
        self.assertNotIn("牛牛1号", message)
        self.assertNotIn("模拟成交", message)
        self.assertNotIn("非实盘", message)
