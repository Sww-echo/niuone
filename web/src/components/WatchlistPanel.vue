...
const sections = computed(() => {
  ...
  // fallback: single section from flat stocks
...
/** Recent trade dates newest-first for "假设这天买入" quick picks. */
...
async function onAddStocks() {
  ...
  await withAdminRetry(() => addStocks(payload))
  if (payload.codes) form.codes = ''
}
  ...
  <section class="card watchlist-panel">
    ...
    <div class="watchlist-toolbar">
      <div class="watchlist-field">
        ...
        <input v-model="form.codes" type="text" inputmode="numeric" placeholder="001896, 600519, 300750" @keydown.enter.prevent="onAddStocks">
      </div>
      <div class="watchlist-field">
      ...
      <div class="watchlist-field grow">
        <span class="field-label">备注</span>
        <input v-model="form.note" type="text" placeholder="可选">
      </div>
      <button type="button" class="primary" :disabled="state.busy" @click="onAddStocks">添加并模拟买入</button>
    ...
    <div class="watchlist-toolbar">
      ...
      <button type="button" :disabled="state.busy" @click="onCreateGroup">创建分组</button>
      <div class="watchlist-field">
    ...
    <div class="watchlist-toolbar filters">
      ...
      <span class="watchlist-meta">
        共 {{ state.board.total }} 只 · {{ tradeDates.length }} 个交易日
      </span>
    </div>

    <div v-if="state.loading && !state.loaded" class="loading">自选股加载中…</div>
    <div v-else-if="!stocks.length" class="empty">还没有自选股，请先创建分组并添加股票代码。</div>

    <div v-else class="watchlist-grouped">
      <div v-for="(section, i) in sections" :key="section.group_id || 'ungrouped'">
          ...
          <table class="watchlist-table">
            ...
            <tbody>
              <tr v-for="stock in section.stocks" :key="stock.code">
                ...
                <td class="buy-cell">
                  <div class="buy-date">{{ stock.buy_date || '—' }}</div>
                ...
                <td class="actions">
                  <button type="button" class="link" @click="openEdit(stock)">改买入日</button>
  ...
  <dialog v-if="edit.open" class="watchlist-dialog" open>
    <form class="watchlist-dialog-form" @submit.prevent="saveEdit">
      ...
      <div class="watchlist-dialog-actions">
        ...
        <button type="submit" class="primary" :disabled="state.busy">按该日重算盈亏</button>
      </div>
    </form>
  </dialog>
